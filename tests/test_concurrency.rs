        ...
        let handle = thread::spawn(move || {
            ...
            let operation_result = match i % 3 {
                ...
                2 => {
                    ...
                    git_clone.for_each_ref("%(refname)", "refs/heads/").map(|_| true)
                }
                _ => Ok(true),
            };
...
fn test_concurrent_branch_checking() -> Result<()> {
    ...
    assert!(!race_detected, "Race condition detected in concurrent branch checking");
...
fn test_concurrent_worktree_operations() -> Result<()> {
    ...
    for handle in handles {
        handle.join().expect("Thread should not panic during worktree operations");
    }
