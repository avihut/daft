    ...
    for url in malicious_urls {
        ...
        match result {
            Ok(repo_name) => {
                ...
                assert!(
                    !repo_name.contains(".."),
                    "Path traversal in extracted name: {}",
                    repo_name
                );
                assert!(
                    !repo_name.contains('\0'),
                    "Null byte in extracted name: {}",
                    repo_name
                );
                assert!(
                    !repo_name.contains(';'),
                    "Command separator in extracted name: {}",
                    repo_name
                );
                assert!(
                    !repo_name.contains('&'),
                    "Command operator in extracted name: {}",
                    repo_name
                );
                assert!(
                    !repo_name.contains('|'),
                    "Pipe operator in extracted name: {}",
                    repo_name
                );
                assert!(
                    !repo_name.contains('<'),
                    "Redirection in extracted name: {}",
                    repo_name
                );
                assert!(
                    !repo_name.contains('>'),
...
fn test_malicious_branch_names() {
    ...
    for branch_name in malicious_branch_names {
        ...
        assert!(
            result.is_err(),
            "Malicious branch name was accepted: {}",
            branch_name
        );
    }
}
...
fn test_malicious_repo_names() {
    ...
    for repo_name in malicious_repo_names {
        ...
        assert!(
            result.is_err(),
            "Malicious repo name was accepted: {}",
            repo_name
        );
    }
}
