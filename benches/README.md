...
One scenario is shaped differently and is **opt-in**, like `vs_competition`:
`bench:copy` (#801) races the `copy:` engine against the platform's own fast
copy (`cp -cR` on macOS, `cp -R --reflink=auto` elsewhere) over a deterministic
pnpm-shaped `node_modules` generated by `benches/fixtures/gen_pnpm_tree.py`. On
macOS it runs two arms — the default clone path and the portable walking path
(`DAFT_COPY_FORCE_WALK`); elsewhere the walking path is already the default, so
the second arm is skipped rather than timing the same command twice.

```bash
mise run bench:copy                 # opt-in; not part of `mise run bench`
SCALE=medium mise run bench:copy    # size the fixture up from the default `small`
```

It stays out of the default suite because it generates and copies a
multi-hundred-MB fixture (minutes and gigabytes of writes, against sub-second
git operations everywhere else) and because it needs `python3`, which nothing
