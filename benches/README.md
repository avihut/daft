...
One scenario is shaped differently: `bench:copy` (#801) races the `copy:` engine
against the platform's own fast copy (`cp -cR` on macOS, `cp -R --reflink=auto`
elsewhere) over a deterministic pnpm-shaped `node_modules` generated by
`benches/fixtures/gen_pnpm_tree.py`, in two arms — the default path and the
portable walking path (`DAFT_COPY_FORCE_WALK`).
`SCALE=medium mise run bench:copy` sizes the fixture up from the default
`small`.
