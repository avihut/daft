...
| ------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `master.json`       | `master`       | No deletion, no force-push, linear history. Changes land only through a pull request, only by squash, only once `ci-gate` (the one required status check, from `.github/workflows/test.yml`) is green, with every review thread resolved. The branch need not be up to date with `master` until public launch (#941; #942 restores that). Repository admins may bypass (that is what keeps `daft merge`'s fast-forward push working for the maintainer). |
