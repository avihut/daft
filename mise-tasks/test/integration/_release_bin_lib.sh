#!/usr/bin/env bash
# Sourceable helper: ensures both release binaries the integration suites
# consume — `daft` and `xtask` — exist under target/release/ before the
# suites run. Prints nothing on stdout the caller depends on.
#
...
# — it does NOT build the `xtask` member. The YAML suite's `runner-output`
...
# this by building both binaries explicitly before the suites
...
ensure_release_binaries() {
  echo "Building release xtask for the integration suites..."
