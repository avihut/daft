...
`cd`, running the same hooks in CI for parity, gating merges on the checks your
forge requires, coordinating work across the repo graph.

<ul>
  <li v-for="r in patterns.filter(p => ['/recipes/declarative-envs', '/recipes/ci-parity', '/recipes/merge-gate-parity', '/recipes/coordinating-service-and-client', '/recipes/polyrepo-monorepo'].includes(p.link))" :key="r.link">
    <a :href="r.link">{{ r.title }}</a> — {{ r.description }}
  </li>
</ul>
