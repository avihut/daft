...
| `--hooks <MODE>`          | How the hook phase runs: `auto` (honor each job's `background:`), `foreground` (run every job inline and wait — a backgrounded gate job otherwise detaches and stops gating), `background` (detach post-merge; the pre-merge gate is exempt and still runs inline), `off` (same as `--skip-hooks all`). Gate policy is unaffected. |
