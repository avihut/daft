...
|--------|-------------|----------|
| `--from <worktree>` | Worktree to copy from, by directory name, branch name, or path (default: the current worktree, or the default branch's worktree when it is the target) |  |
