...
│
├──────────────────────────────────────────────────────────────────┐
│ daft hooks v1.17.0  worktree-post-create  on: daft-652/cool-feature │
├──────────────────────────────────────────────────────────────────┘
┃  bun-install ❯
...
- Lifecycle hooks appear as a plan row; when they actually run, the row expands
  in place into the familiar hook block, welded onto the rail by its `├`
  corners. When nothing is configured to run, the row disappears; skips worth
  noticing (untrusted repository, `--skip-hooks`) render the yellow `↓` row
  instead.
