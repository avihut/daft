...
**`-vv` promotes the step opening line.** When uncapped capture is emitted
inline, the bulk of the screen becomes per-step body content (check labels,
stdout, stderr). Without something to anchor each step block, the eye loses
where one step ends and the next begins. So at `-vv` — and only at `-vv` — the
step name in `[N/M] name ... ok (N checks)` is bold default-fg (the brackets,
`... ok`, and `(N checks)` suffix stay dim/green/dim). At `-v` step names stay
plain: there's no intervening body content to compete with, so bold there would
just add chrome.

This is the canonical example of a verbosity-specific styling rule: a mechanism
that exists exclusively to keep §2 hierarchy legible when one tier emits
dramatically more content than the others. Future rules of this shape get a
one-paragraph callout here; don't bury them in `pretty.rs` helpers without
explaining why.
