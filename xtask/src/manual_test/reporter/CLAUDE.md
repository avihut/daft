...
| **cyan**                | Section heading: scenario name (top-of-block)                                                                                                                                                                                                             | Never status, never expanded command, never a step |
| **bright purple**       | Step identity: step name in the per-step opening line (bold at `-vv` to anchor Layer 2 against Layer 3/4 content, plain at `-v`)                                                                                                                          | Never assertion content                            |
...
| ------------- | ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Primary**   | bold + named color           | Scenario header (bold cyan), scenario footer on FAIL (whole `✗ name` span bold red), `FAIL` word, banner label, `1) ✗ name` in failures block, focal step name in failures block (bold default-fg), **step opening name at `-vv`** (bold bright purple — see §6)                                                                                                                                     |
...
| **Accent**    | named color (may layer bold) | Count numbers (green for passed > 0, red for failed > 0), `(slow)` yellow, semantic icons (`✓` green not bold, `✗` bold red, `❯` bold red), diff labels (`expected:` bold green, `actual:` bold red), **step name in per-step lines** (bright purple at `-v`, bold bright purple at `-vv`), **`$ command` body** (blue, multi-line with `>` continuation lines)                                      |
...
layer its own column; the step opening name bolds in bright purple as the
Layer-2 anchor; `$ command` body is blue with `>` continuation prefixes when the
YAML `run:` field is multi-line (shell prompt convention keeps the visual frame
...
step-color signal above. A blank line separates step blocks. At `-v` the step
name picks up plain bright purple (no bold) so the step-identity color
backports, with the weight reserved for the tier whose Layer 3/4 content makes
the extra anchor necessary.
