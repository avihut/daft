...
- `--hooks <auto|foreground|background|off>` picks how a run's hook phase
  ...
  timeout. Git stages get no flag — git runs the shim — so daft settles it per
  stage: one git blocks on (`pre-commit`, `commit-msg`, `pre-push`, …) runs its
  jobs inline whatever they declared, and the rest (`post-commit`,
  `git-post-merge`, `post-checkout`, `post-rewrite`, …) detach normally.
