...
| `daft repo list [--all] [--columns <COLS>] [--format <FMT>]` | List cataloged repos (name, worktree count, path, remote; `--format json` adds the default branch). `--all` includes removed entries; `--columns` speaks the shared grammar (`+size` adds a disk-usage column, `+layout` the recorded worktree layout, `+branch` the default branch). |
