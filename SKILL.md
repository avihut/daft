...
| `daft git-worktree-sync [-f] [-v\|-vv] [--rebase BRANCH [--autostash]] [--push [--force-with-lease] [--no-verify] [--jobs N] [--no-throttle]] [--include VALUE]... [--stat summary\|lines]` | Synchronize all worktrees: prune stale + update all + optional rebase + optional push (`-f`/`--prune-dirty` for dirty worktrees, `-v` hook details, `-vv` full sequential). By default, rebase and push apply only to branches you own (matching `git config user.email`). Use `--include` to add more branches: `unowned` for all, an email address for a teammate's branches, or a branch name for one specific branch. First Ctrl+C/SIGTERM cancels gracefully: running git subtrees (including pre-push hooks) are terminated by process group, partial results print, and sync exits 130; a second Ctrl+C force-kills. |
...
| `daft repo remove [<path>\|--repo <name>] [--keep-files] [--force\|-y] [--dry-run] [-v]`         | Remove a Git repository entirely: git dir, every checked-out worktree, and the trust marker. Runs `worktree-pre-remove` and `worktree-post-remove` lifecycle hooks per worktree. Prompts before deletion unless `--force`. Refuses paths that are not inside a Git repository. Path defaults to cwd; `--repo <name>` addresses a cataloged repo by name instead. `--keep-files` drops the catalog entry only — no disk changes, no hooks, no prompt; with `--repo` it also retires a stale entry whose directory is already gone. Note: `worktree-post-remove` fires AFTER the worktree directory is gone — `$DAFT_WORKTREE_PATH` no longer exists at that point.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `daft repo list [--all] [--worktrees] [--columns <COLS>] [--format <FMT>]` | List cataloged repos (name, worktree count, path, remote; `--format json` adds the default branch). `--all` includes removed entries; `--worktrees` expands each repo into a tree of its worktrees (branch + path per line; JSON nests a `worktrees` array in place of the count); `--columns` speaks the shared grammar (`+size` adds a disk-usage column, `+layout` the recorded worktree layout, `+branch` the default branch). |
| `daft repo info [<repo>] [--format <FMT>]`                                 | One entry in full — status, path, remote, default branch, recorded layout, and the worktrees as a tree — plus the repo's resolved relations. Accepts name, path, or uuid; defaults to the cwd repo. Identity plumbing (uuid, git common dir, timestamps) and a `{branch, path}` worktrees array live in `--format json`.                                                                                                           |
...
| `daft list\|update\|prune\|doctor --repo <name>`                           | Run the command in another cataloged repo from anywhere. `--all-repos` sweeps every live entry (prune runs the current repo last; doctor adds per-repo sections). `daft list <repo>` is positional sugar for `daft list --repo <repo>` (repo-only resolution — a miss is a hard error, never a branch fallback).                                                                                                                   |
...
invocation. Parallel `sync --push` hook runs are resource-governed (#678): a
memory-aware governor caps concurrent hook-bearing pushes (default max(2,
cores/4); `--jobs N` overrides, `--no-throttle` disables), learns each hook's
peak memory across runs, throttles admissions under memory pressure (rows show
`held: memory`), and under sustained pressure freezes then kills-and-retries the
newest push instead of letting the machine swap. Each push unit also gets a
wall-clock budget (`daft.sync.pushTimeout`, default 30m) so a hung hook cannot
wedge the sync. `daft.sync.pushHookStrategy batched` pushes every branch in one
`git push` so the hook fires once with all refs (one refusal then fails the
whole batch). `--skip-hooks` does NOT affect git-level hooks — it only filters
daft's own jobs.
