...
| `daft start <repo> <branch> [base]`                        | Create a NEW branch in another cataloged repo, based on its default branch unless `[base]` is given. Local-first: an existing local branch named `<repo>` keeps the local reading (`daft start --repo <name> <branch>` is the explicit form). The destination is announced before any work; the target's trust gates its hooks; `-x` runs there, `-c` is refused. |
