...
| `daft repo list [--all] [--sizes] [--format <FMT>]` | List cataloged repos (name, worktree count, path, remote; `--format json` adds the default branch). `--all` includes removed entries; `--sizes` adds a disk-usage column.                                                                              |
