...
| `daft repo remove [<path>] [--force\|-y] [--dry-run] [-v]`                                       | Remove a Git repository entirely: bare git directory, every checked-out worktree, and the trust DB entry. Runs `worktree-pre-remove` and `worktree-post-remove` lifecycle hooks per worktree. Prompts before deletion unless `--force`. Refuses paths that are not inside a Git repository. Path defaults to cwd. Note: `worktree-post-remove` fires AFTER the worktree directory is gone — `$DAFT_WORKTREE_PATH` no longer exists at that point.                                                                                                                                                                                                                                                                                                                                            |
