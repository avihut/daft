...
### `daft repo remove --purge` invalidates your cwd

Running `daft repo remove --purge` from inside the repo being deleted
invalidates the agent's cwd mid-operation. Either pass an explicit target
(`daft repo remove --purge /path/to/repo`) and stay outside, or `cd` to a safe
ancestor first. The binary writes a redirect path to `$DAFT_CD_FILE` for shell
wrappers, but agent shells typically lack that wrapper, so follow-up commands
fail with `chdir: no such file or directory` until the cwd is fixed.

Without `--purge` there is nothing to worry about: the default removal deletes
nothing, so the cwd stays valid.
