...
`daft repo install` creates a starter `daft.yml` at the current worktree root
(commented skeleton with `hooks:`, `shared:`, `layout:` sections). It refuses if
`daft.yml` already exists, pointing the user at `$EDITOR daft.yml`.
