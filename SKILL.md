...
- **The source is ranked against what the new worktree will hold**: the base
  ...
  where you are standing, then to the warmest. The rail's section anchor names
  the winner (`copied paths from 'master'`). The `copy:` **config**, unlike the
  bytes, is read from the worktree you are standing in — a visitor's untracked
  overlay lives there — so a standing worktree whose merged config lacks the key
  plans nothing.
