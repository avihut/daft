...
targets the worktree root, and it refuses (non-zero) when run outside a git
repository or at the bare container root of a contained layout (a `daft.yml`
there is inert — no worktree reads it). If a `daft.yml` already exists it does
not hard-error: it reports whether that file is tracked (a committed team
baseline) or a visitor config (untracked, private to this clone) and stops,
suggesting `daft.local.yml` for personal overrides.
