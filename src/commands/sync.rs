...
starts, every running git subprocess is terminated by process group (reaching
a pre-push hook and all of its descendants, even ones that moved to their own
process groups or were stopped by terminal job control), an interrupted rebase
is aborted to restore the worktree, and sync prints partial results and exits
130. A second Ctrl+C force-kills anything still running and exits immediately.
