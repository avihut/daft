...
//! the per-repo redb `JobStore` as JSON, one row per line.
//!
//! Subcommands:
//! - `repo-policy` — the single row in `repo_policy_v1` for the repo
...
//! grep'd the sidecar continue to work after the JSON→redb cutover.
//!
...
//! scenarios a stable read path into redb without re-implementing the
...
fn dump_repo_policy() -> Result<()> {
    ...
    let store = crate::coordinator::store::JobStore::open_for_repo_base(&repo_hash, &base)
