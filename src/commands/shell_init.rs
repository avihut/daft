...
# Resolve the daft binary on every invocation so a rebuild (e.g. via
# `mise run dev` regenerating target/release/daft) is picked up without
# re-sourcing. Run `command -v` in a subshell with the daft() function
# unset — bash/zsh `command -v` returns the function name (just "daft")
# once daft() is defined, which is useless as an executable path.
__daft_find_bin() {
    (unset -f daft 2>/dev/null; command -v daft 2>/dev/null)
}
