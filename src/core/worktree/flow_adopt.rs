...
pub fn execute(
    ...
    if params.dry_run {
        ...
        sink.on_step("[DRY RUN] Convert .git to bare repository");
        sink.on_step(&format!(
            "[DRY RUN] Register worktree for branch '{current_branch}'"
        ));
        if git.has_uncommitted_changes()? {
            sink.on_step("[DRY RUN] Restore uncommitted changes in new worktree");
        }
    ...
    let result = transform::convert_to_bare(&convert_params, sink)?;
