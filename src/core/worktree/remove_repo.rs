...
/// `gix::discover` + `repo.common_dir()`; the `project_root` is taken as the
...
pub fn resolve_repo(path: Option<&Path>) -> Result<RepoTarget> {
    ...
    // gitoxide's `repo.common_dir()` can come back relative to the *process*
    // cwd, and with a relative `start` the join below double-prefixed
    // (`<rel-start>/<rel-start>/.git`). Anchoring `start` to an absolute path
    // removes the ambiguity. Regression: bash test
    // `repo_remove_relative_path_from_parent`.
    let start = std::fs::canonicalize(&start)
        .with_context(|| format!("could not canonicalize {}", start.display()))?;

    let common_dir = gix::discover(&start)
...
///
/// Opens the repo via `gix::open` and combines `repo.workdir()` (main
/// worktree) with `repo.worktrees()` (linked worktrees); the bare entry is
/// never included.
///
...
pub fn enumerate_worktrees(target: &RepoTarget) -> Result<Vec<WorktreeEntry>> {
    ...
    for proxy in repo.worktrees().context("gix repo.worktrees() failed")? {
        ...
        // `git worktree list --porcelain` would still emit the stale entry,
        // but for removal that's moot: the bare dir's `worktrees/<name>/`
        // admin entry is destroyed when we `fs::remove_dir_all` the bare's
        // git_common_dir afterwards, so skipping the entry here just means we
        // don't attempt a redundant (and impossible) filesystem removal of a
...
mod tests {
    ...
    fn resolve_repo_handles_relative_path_from_parent() {
        ...
        // CI regression: `daft repo remove <relative-path>` from the parent
        // dir failed with "could not canonicalize <rel>/<rel>/.git" —
        // gitoxide's `repo.common_dir()` returned a path relative to the
        // *process* cwd, not relative to `start`, so the naive
        // `start.join(common_dir)` double-prefixed the directory.
        let tmp = tempfile::tempdir().unwrap();
        ...
        let result = resolve_repo(Some(Path::new("myrepo")));
        // Restore before asserting so a panic doesn't leak cwd into other
        // tests in the same process.
        std::env::set_current_dir(prev_cwd).unwrap();

        let target = result.unwrap_or_else(|e| panic!("relative path failed: {e:#}"));
    ...
    fn enumerate_worktrees_lists_main_and_linked() {
        ...
        let target = resolve_repo(Some(tmp.path())).unwrap();
        let mut worktrees = enumerate_worktrees(&target).unwrap();
