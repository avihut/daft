...
impl GitCommand {
    ...
    pub fn symbolic_ref_short_head(&self) -> Result<String> {
        oxide::symbolic_ref_short_head(&self.gix_repo()?)
    }
    ...
    /// inside a git hook. Subprocess-only by design — gitoxide has no
    /// `--not --remotes` walk (same precedent as `merge_base` and
    /// `commit_tree` below).
